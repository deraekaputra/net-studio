    ...
    <div>
        ...
        <div class="sign">
            <div class="sign__wrapper">
                <h6 class="sign__title">Daftar sekarang untuk mendapatkan akses studio acara NET. favoritmu.</h6>
                <span class="sign__error" v-show="isError" v-if="errorType === 'empty'">Nama lengkap dan nomor seluler tidak boleh kosong</span>
                <span class="sign__error" v-show="isError" v-else-if="errorType === 'alreadyRegistered'">Nama lengkap dan nomor seleler sudah terdaftar</span>
                <div class="sign__input">
                    <div class="form-input">
                        <div class="form-input__wrapper">
                            <input v-model="namaLengkap" v-on:keyup="checkValidation" class="form-input__field" type="text" placeholder="Nama lengkap" :class="[ isError ? 'form-input__field--error' : ''  ]">
                            <span class="form-input__placeholder">Nama lengkap</span>
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="form-input__wrapper">
                            <input v-model="nomorSeluler" v-on:keyup="checkValidation"  class="form-input__field" type="text" placeholder="0812-XXXX-XXXX" :class="[ isError ? 'form-input__field--error' : ''  ]" @keypress="restrictChars">
                            <span class="form-input__placeholder">Nomor seluler</span>
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="form-input__wrapper">
                            <input v-model="emailUser" v-on:keyup="checkValidation" class="form-input__field" type="email" placeholder="nama@email.com" :class="[ isError ? 'form-input__field--error' : ''  ]">
                            <span class="form-input__placeholder">Email</span>
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="form-input__wrapper">
                            <input v-model="password" v-on:keyup="checkValidation" class="form-input__field" type="Password" placeholder="***********" :class="[ isError ? 'form-input__field--error' : ''  ]">
                            <span class="form-input__placeholder">Password</span>
                        </div>
                    </div>  
                    <div class="form-input">
                        <div class="form-input__wrapper">
                            <input v-model="confirmationPassword" v-on:keyup="checkValidation" class="form-input__field" type="Password" placeholder="***********" :class="[ isErrorPassword ? 'form-input__field--error' : ''  ]">
                            <span class="form-input__placeholder">Confirm Password</span>
                            <span class="sign__error inline-error" v-show="isErrorPassword">Password tidak sama</span>
                        </div>
                    </div> 
                </div>
                <button class="sign__button" :disabled="isDisable == true" type="button">Daftar</button>
                <span class="sign__link">Sudah punya akun? <a href="/signin">Masuk</a></span>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            checkValidation() {
                if (this.namaLengkap === '' || this.nomorSeluler === '' || this.password === '' || this.confirmationPassword === '' || this.emailUser === '' ) {
                    console.log('kosong')
                    this.isDisable = true
                } else if (this.confirmationPassword.length > 0 && this.confirmationPassword !== this.password  ) {
                    this.isErrorPassword = true
                    this.isDisable = true
                } else {
                    console.log('ada')
